...
int main() {
    ...
    float* d_test;
    cudaMalloc(&d_test, 100000000 * sizeof(float));
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        // FIX Bug 4: read loss and outputs AFTER backward + optimizer step
        float h_loss;
        loss.toHost(&h_loss);

        float* h_out = new float[batch * out_f];
        out.toHost(h_out);   // ← moved to after optimizer.step

        cout << "Epoch " << epoch << " Loss: " << h_loss << "  Output: ";
        for (int i = 0; i < batch * out_f; i++) cout << h_out[i] << " ";
        cout << endl;
